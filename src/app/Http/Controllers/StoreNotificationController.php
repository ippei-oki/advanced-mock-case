...
class StoreNotificationController extends Controller
{
    ...
    {
        $users = User::all();
        return view('store.notifications.index', compact('users'));
    }
    ...
    public function send(Request $request)
    {
        ...
        if ($request->recipients === 'all') {
            ...
            return redirect()->route('store.notifications.index')->with('error', '送信先が選択されていません。');
        }
        ...
        return redirect()->route('store.notifications.index')->with('success', 'お知らせメールを送信しました。');
    }
}
